...
logger = logging.getLogger("seed-massive-flights-batch")
...
def seed_flights(airports, airlines):
    ...
    # Bidirectional Routes with 5 flights per day each
    routes = [
        # Domestic India
    ...
    for day_offset in range(0, 31):
        ...
        for route in routes:
            ...
            for f in route["flights"]:
                ...
                arr_time_parsed = datetime.strptime(f['time'], '%H:%M') + timedelta(hours=3)
                arr_datetime = f"{date_str}T{arr_time_parsed.strftime('%H:%M')}:00+00:00"
    ...
    logger.info(f"Preparing to upsert {len(flights_to_insert)} flights in batches...")
    
    # Upsert flights in chunks of 100
    upserted_flights = []
    chunk_size = 100
    for i in range(0, len(flights_to_insert), chunk_size):
        chunk = flights_to_insert[i:i+chunk_size]
        res = supabase.table("pss_flights").upsert(chunk, on_conflict="flight_number,departure_datetime").execute()
        if res.data:
            upserted_flights.extend(res.data)
            logger.info(f"Upserted flights chunk {i // chunk_size + 1}: {len(res.data)} rows returned.")
            
    # Map (flight_number, departure_datetime) -> flight_id
    flight_id_map = {}
    for f in upserted_flights:
        dep_str = f["departure_datetime"]
        # Normalize timestamp format returned by Supabase (sometimes ends with +00, sometimes Z, sometimes has microseconds)
        # We will parse departure_datetime as ISO datetime
        try:
            parsed_dep = datetime.fromisoformat(dep_str.replace("Z", "+00:00"))
            dep_key = (f["flight_number"], parsed_dep.isoformat())
            flight_id_map[dep_key] = f["flight_id"]
        except Exception as e:
            logger.warning(f"Failed to parse datetime {dep_str}: {e}")
            
    # Also create key formats for the exact local inputs
    for f in upserted_flights:
        # Standard format match
        dep_str = f["departure_datetime"]
        # Match "2026-07-15T07:00:00+00:00" from dep_datetime
        # Let's normalize it to first 19 chars + +00:00
        norm_dep = dep_str[:19] + "+00:00"
        flight_id_map[(f["flight_number"], norm_dep)] = f["flight_id"]
        # Also direct key match
        flight_id_map[(f["flight_number"], dep_str)] = f["flight_id"]
        
    logger.info(f"Constructed mapping for {len(flight_id_map)} flight keys.")
    
    # Prepare inventories and fares
    ...
    for day_offset in range(0, 31):
        ...
        for route in routes:
            ...
            for f in route["flights"]:
                ...
                # Retrieve flight_id
                f_id = flight_id_map.get((f["num"], dep_datetime))
                if not f_id:
                    # Try other formats
                    # Let's match by searching with list comprehension
                    matched = [fid for (fnum, d_dt), fid in flight_id_map.items() if fnum == f["num"] and d_dt.startswith(date_str) and f["time"] in d_dt]
                    if matched:
                        f_id = matched[0]
                ...
                for b_class, cab_class, multiplier in [('Y', 'economy', 1.0), ('B', 'economy', 0.8), ('J', 'business', 3.5)]:
                    fares_to_insert.append({
                        ...
                        "cabin_class": cab_class,
                        "booking_class": b_class,
                        "base_fare_usd": float(f["price"] * multiplier),
                        "valid_from": date_str
                    })
                    
    logger.info(f"Upserting {len(inventories_to_insert)} inventories in batches...")
    for i in range(0, len(inventories_to_insert), chunk_size):
        chunk = inventories_to_insert[i:i+chunk_size]
        try:
            supabase.table("pss_inventory").upsert(chunk, on_conflict="flight_id,booking_class").execute()
        except Exception as e:
            logger.warning(f"Error upserting inventory chunk {i // chunk_size}: {e}")
            
    logger.info(f"Upserting {len(fares_to_insert)} fares in batches...")
    for i in range(0, len(fares_to_insert), chunk_size):
        chunk = fares_to_insert[i:i+chunk_size]
        try:
            supabase.table("pss_fares").upsert(chunk, on_conflict="fare_basis_code,origin_airport_id,destination_airport_id,booking_class,airline_id").execute()
        except Exception as e:
            logger.warning(f"Error upserting fares chunk {i // chunk_size}: {e}")
            
    logger.info("Bulk seeding completed successfully!")

def main():
    logger.info("Starting massive flight batch seeding...")
